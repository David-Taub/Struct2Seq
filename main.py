...
class Learner(object):
    ...
    def __enter__(self):
        ...
        self.Struct2Seq = Struct2Seq(self.verbose)
        ...
        self.mainQN = QNetwork(self.Struct2Seq.get_state_sizes(), self.Struct2Seq.action_size())
        # self.targetQN = QNetwork(self.Struct2Seq.get_state_sizes(), self.Struct2Seq.action_size())
    ...
    def _gen_action(self, state):
        is_explore_step = np.random.rand(1)[0] < self.explore_prob or self.total_steps < PRE_TRAIN_STEPS
        if is_explore_step:
            action = self._gen_rand_action();
        else:
    ...
    def _run_step(self, state):
        ...
        next_state, reward = self.Struct2Seq.step(action)
        ...
        self.total_steps += 1
        if len(self.episode_rollout) % STEPS_BETWEEN_DQN_TRAIN == 0:
            self._batch_train_DQN()
        state = new_state
    ...
    def _batch_train_DQN(self):
        if len(self.episode_rollout) < 3:
            return
        state = np.reshape(self.Struct2Seq.get_state(), [1, -1])
    ...
    def _run_episode(self):
        ...
        state = self.Struct2Seq.reset()
        self._run_step(state)
        self.episodes_scores.append(self.Struct2Seq.score())

    def show(self):
        self.Struct2Seq.reset()
